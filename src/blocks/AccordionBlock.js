...
export class AccordionBlock {
  ...
  addItem() {
    ...
    const newQuestionInput = this.itemsContainer.children[newIndex]?.querySelector(".cdx-accordion-question");
    if (newQuestionInput) {
   ...
   */
  onPaste(event) {
    const { detail: { data: pasteData } } = event;
    
    if (pasteData.tagName === "DETAILS") {
      const details = pasteData.node;
      const summary = details.querySelector("summary");
      const content = details.querySelector(".accordion-content, [class*='accordion']");
