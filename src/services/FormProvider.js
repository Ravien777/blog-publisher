  ...
  async detect() {
    ...
    try {
      ...
      if (!res.ok && res.status === 404) {
        ...
        res = await fetch(
          `${this.apiUrl}/wp-json?rest_route=${restRoute}&per_page=50`,
          {
            headers: this.headers,
          },
        );
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data)) {
          ...
          formsArray = Object.values(data).filter(
    ...
    if (!this.plugin) {
      ...
      try {
        ...
        if (!res.ok && res.status === 404) {
          ...
          const restRoute = encodeURIComponent(
            "/contact-form-7/v1/contact-forms",
          );
          res = await fetch(
            `${this.apiUrl}/wp-json?rest_route=${restRoute}&per_page=100`,
            {
              headers: this.headers,
            },
          );
